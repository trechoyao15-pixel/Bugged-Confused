...
<head>
  <meta charset="utf-8" />
  <title>Report Lost Item — LTMS</title>
    ...
    <section class="wrap form-page">
      <div class="form-card">
        <header class="form-card-header">
          ...
          <p class="muted">Provide as much detail as possible to help recovery.</p>
        </header>

        <?php if (!$logged_in): ?>
        <?php endif; ?>

        <form id="reportForm" action="save_lost.php" method="POST" enctype="multipart/form-data" class="report-form" novalidate>
          <?php if ($form_message): ?>
          ...
          <div class="form-row">
            ...
            <textarea id="description" name="description" rows="4" required placeholder="Describe distinguishing features, brand, contents, etc."></textarea>
          </div>
          ...
          <div class="form-grid">
            ...
            <div class="form-row">
              ...
              <input id="location_lost" name="location_lost" type="text" required placeholder="e.g. Library - Main Entrance">
            </div>
          </div>
          ...
          <div class="form-actions">
            ...
            <a class="btn btn-ghost" href="lost_list.php">View Lost Items</a>
          </div>
  ...
  (function () {
    ...
    if (form) {
      form.addEventListener('submit', async (e) => {
        ...
        const dateLost = form.querySelector('input[name="date_lost"]').value;
        const locationLost = form.querySelector('input[name="location_lost"]').value.trim();

        if (!title || !description || !dateLost || !locationLost) {
        ...
        try {
          ...
          if (data) {
            ...
            if (text.trim().length === 0) {
              msg.style.color = 'crimson';
              msg.textContent = 'No response from server. Check server logs or ensure save_lost.php exists.';
            } else {
              msg.style.color = 'crimson';
